...
function getServiceHost(workspacePath, jsDocuments) {
    ...
    const parsedConfig = ts.parseJsonConfigFileContent(configJson, ts.sys, workspacePath, compilerOptions, configFilename, undefined, [{ extension: 'vue', isMixedContent: true }]);
    const files = parsedConfig.fileNames;
    const isOldVersion = inferIsOldVersion(workspacePath);
    compilerOptions = parsedConfig.options;
    compilerOptions.allowNonTsExtensions = true;
    function updateCurrentTextDocument(doc) {
        const fileFsPath = preprocess_1.getFileFsPath(doc.uri);
        const filePath = preprocess_1.getFilePath(doc.uri);
