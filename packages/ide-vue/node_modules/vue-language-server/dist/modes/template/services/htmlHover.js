...
const TRIVIAL_TOKEN = [
    htmlScanner_1.TokenType.StartTagOpen, htmlScanner_1.TokenType.EndTagOpen, htmlScanner_1.TokenType.Whitespace
];
        ...
        for (const provider of tagProviders) {
            ...
            provider.collectTags((t, label) => {
                if (t === tag) {
                    ...
                    hover = { contents: [{ language: 'html', value: tagLabel }, vscode_languageserver_types_1.MarkedString.fromPlainText(label)], range };
                }
            });
            if (hover) {
                return hover;
    ...
    function getAttributeHover(tag, attribute, range) {
        ...
        for (const provider of tagProviders) {
            provider.collectAttributes(tag, (attr, type, documentation) => {
                ...
                const contents = [
                    documentation ? vscode_languageserver_types_1.MarkedString.fromPlainText(documentation) : `No doc for ${attr}`
                ];
