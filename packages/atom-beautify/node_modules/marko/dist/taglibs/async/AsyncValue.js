...
AsyncValue.prototype = {
     ...
     * available.
     */
    bA_: function (callback) {
        // Do we already have data or error?
        if (this.bz_) {
            // invoke the callback immediately
            return callback(this.bx_, this.az_);
        }

        var callbacks = this.by_ || (this.by_ = []);
    ...
    bC_: function (value) {
        ...
        if (value && typeof value.then === "function") {
            ...
            var finalPromise = value.then(function onFulfilled(value) {
                nextTick(asyncValue.bC_.bind(asyncValue, value));
            }, function onRejected(err) {
                nextTick(asyncValue.bB_.bind(asyncValue, err));
            });
