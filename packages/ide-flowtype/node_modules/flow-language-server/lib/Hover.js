...
class HoverSupport {
  ...
  provideHover(params) {
    ...
    return (0, _asyncToGenerator2.default)(function* () {
      ...
      const fileName = (0, _util.fileURIToPath)(textDocument.uri);
      const doc = _this.documents.get(textDocument.uri);

      const typeHint = yield _this.flow.typeHint(fileName, doc.buffer, (0, _util.lspPositionToAtomPoint)(position));

      if (typeHint) {
        return {
          contents: typeHint.hint.map(function (hint) {
            return hint.type === 'snippet' ? {
              language: 'javascript',
              value: hint.value
            } : hint.value;
          }),
          range: (0, _util.atomRangeToLSPRange)(typeHint.range)
        };
      }
