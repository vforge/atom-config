        ...
        if ((resourceString === 'vscode://defaultsettings/keybindings.json') || Strings.endsWith(resourceString.toLowerCase(), '/user/keybindings.json')) {
            if (root.type === 'array') {
                ...
                root.items.forEach((item) => {
                    if (item.type === 'object') {
                        let property = item.getFirstProperty('key');
                        if (property && property.value) {
                            let location = vscode_languageserver_types_1.Location.create(document.uri, vscode_languageserver_types_1.Range.create(document.positionAt(item.start), document.positionAt(item.end)));
                            result.push({ name: property.value.getValue(), kind: vscode_languageserver_types_1.SymbolKind.Function, location: location });
                        }
                    }
                });
                return result;
